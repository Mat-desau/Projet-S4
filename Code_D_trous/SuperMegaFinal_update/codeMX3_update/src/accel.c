...
void ACL_Init()
{
    ...
    ACL_SetRegister(ACL_CTRL_REG1, 0x9);        // Output data rate at 800Hz, no auto wake, no auto scale adjust, no fast read mode
    //ACL_SetRegister(ACL_INT_SOURCE, 1);
}
...
int raw2int(unsigned char *rgVals)
{
    int result = 0;
    result = ((int)(rgVals[0] << 4)) + (rgVals[1] >> 4);
    if(result & (1 << 11))
    {
        result |= 0xFFFFF000;
    }
    return result;
    //return (((signed int)rgVals[0]<<24)>>20  | (rgVals[1] >> 4)); //VR
    
}

void task_createPaquet(int * count)
{
    /*format UDP_Send_Buffer
     * [0] = header
     * [1-40] : valeur X
     * [41-80]:valeur Y
     * [81-120]: Valeur Z
     */
        ...
        if (*count < 40)
        {
        UDP_Send_Buffer[4 + *count * 4] = raw2int(accel_buffer) >> 24;   //signe repete
        UDP_Send_Buffer[5 + *count * 4] = raw2int(accel_buffer) >> 16;   //signe repete
        UDP_Send_Buffer[6 + *count * 4] = raw2int(accel_buffer) >> 8 ;   //4 MSB + signes repete
        UDP_Send_Buffer[7 + *count * 4] = raw2int(accel_buffer);         //8 LSB
        
        UDP_Send_Buffer[164 + (*count * 4)] = raw2int(accel_buffer + 2) >> 24;   //signe repete
        UDP_Send_Buffer[165 + (*count * 4)] = raw2int(accel_buffer + 2) >> 16;   //signe repete
        UDP_Send_Buffer[166 + (*count * 4)] = raw2int(accel_buffer + 2) >> 8 ;   //4 MSB + signes repete
        UDP_Send_Buffer[167 + (*count * 4)] = raw2int(accel_buffer + 2);         //8 LSB
        
        UDP_Send_Buffer[324 + (*count * 4)] = raw2int(accel_buffer + 4) >> 24;   //signe repete
        UDP_Send_Buffer[325 + (*count * 4)] = raw2int(accel_buffer + 4) >> 16;   //signe repete
        UDP_Send_Buffer[326 + (*count * 4)] = raw2int(accel_buffer + 4) >> 8 ;   //4 MSB + signes repete
        UDP_Send_Buffer[327 + (*count * 4)] = raw2int(accel_buffer + 4);         //8 LSB
        *count = *count + 1;
        
           /*SYS_CONSOLE_PRINT("\r\ncount: \tX_avg = %d",((UDP_Send_Buffer[4] << 24) | (UDP_Send_Buffer[5] << 16) | (UDP_Send_Buffer[6] << 8) | (UDP_Send_Buffer[7])));
           SYS_CONSOLE_PRINT("\tY_avg = %d",((UDP_Send_Buffer[164] << 24) | (UDP_Send_Buffer[165] << 16) | (UDP_Send_Buffer[166] << 8) | (UDP_Send_Buffer[167])));
           SYS_CONSOLE_PRINT("\tZ_avg = %d",((UDP_Send_Buffer[324] << 24) | (UDP_Send_Buffer[325] << 16) | (UDP_Send_Buffer[326] << 8) | (UDP_Send_Buffer[327])));*/
        }
        else
        {
            
            UDP_Send_Buffer[0] = entete_counter >> 24; //l'entete du paquet
