...
int Mode_Lumiere = 0;                   //Mode de la lumiere : (0 = Lorsque la lumière est en haut du threshold pendant plus de 5 minutes, 1 = Lorsque la lumière est sous le threshold pendant plus de 5 minutes)
unsigned int Valeur_Threshold = 50;     //Valeur du threshold afficher sur LCD ligne 2
unsigned int Valeur_Lumiere = 0;                 //Valeur de la lumiere lu par la cellule photolumineuse
...
int FCT_Sur100(int Valeur_Entree, int Valeur_Max);  //Adapte une entrée sur 100
void FCT_Lecture_Lumiere();                         //Lecture de lumière avec sortie sur 100
int FCT_Comparer_Lumiere();                         //Compare la luimière avec threshold
...
{
    LCD_WriteStringAtPos("                ", 0, 0);     //Clear ecran ligne 0
...
void FCT_Affichage_Lumiere()
{
    ...
    char VALEUR[5];
    sprintf(VALEUR, "%d", Valeur_Lumiere);
    LCD_WriteStringAtPos("LUM:", 1, 7);
    LCD_WriteStringAtPos(VALEUR, 1, 11);
    
    sprintf(VALEUR, "%d", Mode_Lumiere);
    LCD_WriteStringAtPos("M", 1, 14);
    LCD_WriteStringAtPos(VALEUR, 1, 15);
}
...
void FCT_Afficher_Threshold() 
{
    ...
    LCD_WriteStringAtPos("TR:", 1, 0);
    LCD_WriteStringAtPos(VALEUR, 1, 3);
}
...
int FCT_Compteur_5Minutes(int Activer)
{
    ...
    if(count5min >= 6)            //Si par dessus 400 c'est bon pour return 1
    {
        return 1;
    }
...
{
    Valeur_Lumiere = FCT_Sur100(ADC_AnalogRead(24), 1024); 
}
...
int FCT_Comparer_Lumiere()
{
    ...
    else if (Valeur_Lumiere < Valeur_Threshold)
    {
        ...
        LED_SetValue(6, 0);
        LED_SetValue(7, 0);
        FCT_Compteur_5Minutes(0);                //Désactive le compteur si Lumiere plus petit que threshold
    }
    if (FCT_Compteur_5Minutes(1) == 1)          //5 minutes ont passées
    {
